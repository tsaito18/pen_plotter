'''
Dumb wrapper to call hta.idraw_merge as an inkscape extension.
...
idraw_merge = from_dependency_import('hta.axidraw_merge')
...
if __name__ == '__main__':
    ...
    try:
        ...
        conf = import_module("idraw_conf") # Import axidraw_conf.py from top level of extensions
        merge_conf = import_module("idraw_merge_conf") # Import axidraw_conf.py from top level of extensions
        e = idraw_merge.AxiDrawMergeClass(ad_params=conf, params=merge_conf, default_logging=False)
        ...
        if "axidraw_conf" == "notamodule":
            ...
            e = idraw_merge.AxiDrawMergeClass(default_logging=False)
        else:
            raise
