...
try: # imported as a python module, e.g. imported from a wrapper (see axidrawforinkscape repo)
...
common_axi = from_dependency_import('axidrawinternal.axidraw_options.common_options')
...
class AxiDrawMergeClass( inkex.Effect ):
    '''AxiDraw Merge Main Class'''

    def __init__( self, default_logging = True, params = None, ad_params = None):
        if params is None:
            params = import_module("hta.axidraw_merge_conf") # Default configuration file
        self.params = params

        if ad_params is None:
            ad_params = import_module("axidrawinternal.axidraw_conf") # Default config file
        self.ad_params = ad_params # pass-through AxiDraw config file data
        ...
        self.version_string = "AxiDraw Merge v 5.5.0, 2023-05-11"
    ...
    def effect( self ):
        ...
        ad = axidraw.AxiDraw(params=self.ad_params)
        ...
        if self.options.mode != "csv":
            ...
            self.require_hard_revert = True # True if we preparse, but haven't run AxiDraw to plot.
        ...
        if self.options.mode == "auto_plot":
            ...
            if self.options.resume_type == "home":
                ...
                self.merge_and_plot(hta, ad)
            elif ad.plot_status.resume.old.application != "axidraw merge":
            ...
            if self.options.preview:
                ...
                selected_options = {item: self.options.__dict__[item] for item in ['pen_pos_up',
                    'pen_pos_down', 'pen_rate_raise', 'pen_rate_lower', 'penlift',]}
    ...
    def merge_and_plot( self, ha_ref, ad_ref ):
        ...
        4. Collect plotdata output from axidraw.py and modify
        ...
        # 3. Plot the document, using axidraw.py. Many options to pass through.
        ...
        selected_options = {item: self.options.__dict__[item] for item in ['pen_pos_up',
            ...
            'report_time', 'resolution', 'preview', 'rendering', 'model', 'penlift',
        ...
        ad_ref.effect() # Plot the document using axidraw.py
        self.rows_plotted += 1
        self.require_hard_revert = False
    ...
    def modify_plotdata( self ):
        ...
        if data_node is not None:
            try:
                data_node.set('application', "axidraw merge")      # Name of this program
