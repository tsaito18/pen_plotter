...
### 滑らかさの確保
- **推論時スムージング**: オフセット予測後に1D移動平均（kernel_size=11）を適用
- **オフセットクランプ**: ±0.3に制限し、大きな変形によるジャギーを防止
...
- [x] F. スムージング（推論時移動平均kernel=11 + クランプ±0.3）
...
### 現在の状態（2026-03-24）
- ユーザーデータのみで訓練した v3-user モデルが動作中
- 文字は読めるが、KanjiVGの形に近すぎる（スタイル表現が不足）
- オフセットクランプ±0.3 + スムージングkernel=11 が現在のスイートスポット
- 137文字 / 269サンプル収集済み

### 残りTODO
- [ ] 文字品質向上: ユーザーの書き癖をもっと反映（現状KanjiVGに近すぎる）
- [ ] ユーザーサンプル追加収集（現在137文字→200-300文字目標）
...
### 次のアクション
1. **文字品質向上** — クランプ緩和・モデル改善・サンプル追加で書き癖反映度を上げる
2. **ユーザーサンプル追加収集** — collect_strokes.py で200-300文字に拡大
3. **数式レイアウト統合** — math_layout.py をWeb UIパイプラインに接続
4. **実機テスト** — G-code出力→ペンプロッタで印刷
