...
#### ホーミング＆座標系
ホーミング（`$H`）で紙の右上角に移動。作業エリアは負の方向に広がる。
紙座標で正の値を使うために `G92` でオフセットを設定する。

**初期化シーケンス:**
```gcode
$H                          ; ホーミング（リミットスイッチまで移動）
G1G91 X210 Y0 F5000        ; 作業原点（紙の右上角付近）に相対移動
G4 P1                       ; 安定待ち
G92 X210 Y297 Z0            ; 現在位置を紙座標(210,297)に設定
G90                          ; 絶対座標モード
