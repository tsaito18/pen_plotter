...
### 現在の状態（2026-03-24）
- v3-user モデル動作中（局所曲率特徴追加、クランプ±1.2、loss=10.4）
- スタイルサンプル正接続済み（ゼロベクトル問題修正）
- augmentation→typesetter統合済み（ベースライン揺らぎ・字間・サイズ変動）
- ストローク太さ変化（始筆太→終筆細、控えめ設定）
- 段落インデント・font_size 6.0mm・行密度変動・文字間シフト実装済み
- 137文字 / 269サンプル収集済み
- 423テスト全パス

### 残りTODO
- [ ] ユーザーサンプル追加収集（現在137文字→200-300文字目標）— スタイル表現力に最も直結
...
### 次のアクション
1. **ユーザーサンプル追加収集** — collect_strokes.py で200-300文字に拡大（品質向上の最短ルート）
2. **数式レイアウト統合** — math_layout.py をWeb UIパイプラインに接続
3. **実機テスト** — G-code出力→ペンプロッタで印刷
4. **セクション見出し・ページ番号** — レポートとしての体裁
