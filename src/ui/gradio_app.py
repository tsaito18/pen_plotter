...
| ブロック図 | ` ```blockdiagram `<br>`R --> [制御器] --> C`<br>`feedback: [センサ]`<br>` ``` ` | 制御ブロック線図。`feedback:` の有無でオープン/クローズドループ |
| デンドログラム | ` ```dendrogram `<br>`order: B C A D E`<br>`B + C : 1`<br>` ``` ` | 階層的クラスタリング図。U字ブラケット＋高さ軸で描画 |
| 段落区切り | 空行 | 空行で段落を分割 |
